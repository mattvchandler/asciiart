...
inline bool is_jpeg(const Image::Header & header)
{
    ...
    const std::array<unsigned char, 4>  jpeg_header2_1 = {0xFF, 0xD8, 0xFF, 0xE0};
    const std::array<unsigned char, 5>  jpeg_header2_2 = {0x4A, 0x46, 0x49, 0x46, 0x00}; // there are 2 "don't care" bytes between these
    ...
    return std::equal(std::begin(jpeg_header1),   std::end(jpeg_header1),   std::begin(header), Image::header_cmp)
       ||  (std::equal(std::begin(jpeg_header2_1),   std::end(jpeg_header2_1),   std::begin(header), Image::header_cmp)
            && std::equal(std::begin(jpeg_header2_2), std::end(jpeg_header2_2), std::begin(header) + std::size(jpeg_header2_1) + 2, Image::header_cmp))
